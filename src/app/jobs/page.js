        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          {jobs?.map(job => (
            <Link key={job.id} href={`/jobs/${job.job_categories?.slug}/${job.id}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', borderRadius: '14px', padding: '0.6rem 0.875rem', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', gap: '10px', borderLeft: '3px solid #1a3c8f' }}>
