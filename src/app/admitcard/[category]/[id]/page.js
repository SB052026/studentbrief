...
export default async function AdmitCardDetailPage({ params }) {
      ...
      <div className="page-wrapper">
        ...
        <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ fontWeight: 700, color: '#475569' }}>Admit card nahi mila</h2>
            <Link href="/admitcard" style={{ color: '#1a3c8f', marginTop: '8px', display: 'block' }}>← Wapas jao</Link>
          </div>
        </main>
        <Footer />
      </div>
  ...
  const dateItems = [
    ...
    { label: 'Download Last Date', value: formatDate(item.download_last_date) },
  ]

  const detailCards = [
    { icon: '🏢', title: 'Organization', value: item.organization, bg: '#dbeafe' },
    { icon: '📋', title: 'Post Name', value: item.post_name, bg: '#dcfce7' },
    { icon: '📊', title: 'Status', value: item.admitcard_status, bg: '#fef3c7' },
  ]
    ...
    <div className="page-wrapper">
      ...
      <main style={{ flex: 1, maxWidth: '800px', margin: '0 auto', width: '100%', padding: '1.5rem 1rem' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px', fontSize: '0.75rem', marginBottom: '1rem' }}>
          ...
          <Link href={`/admitcard/${category}`} style={{ color: '#1a3c8f', textDecoration: 'none' }}>{item.admitcard_categories?.name}</Link>
        </div>
        ...
        <a href={item.admitcard_link} target="_blank" rel="noopener noreferrer" style={{
