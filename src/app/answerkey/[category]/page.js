      ...
      <main style={{ flex: 1, maxWidth: '900px', margin: '0 auto', width: '100%', padding: '1.5rem 1rem' }}>
        ...
        {answerkeys && answerkeys.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {answerkeys.map((item) => (
              <Link key={item.id} href={`/answerkey/${category}/${item.id}`} style={{ textDecoration: 'none' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                    <div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', fontSize: '0.72rem', color: '#64748b' }}>
                        <span>📅 Objection Last Date: {formatDate(item.objection_last_date)}</span>
                      </div>
                    </div>
          ...
          <EmptyState title="Koi answer key nahi mili" description="Is category me abhi koi answer key available nahi hai" icon="📋" />
        )}
      </main>
      <Footer />
    </div>
