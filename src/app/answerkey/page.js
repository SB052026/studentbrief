...
export const metadata = { title: 'Answer Keys - StudentBrief' }
  ...
  return (
    <div className="page-wrapper">
      ...
      <main style={{ flex: 1, maxWidth: '900px', margin: '0 auto', width: '100%', padding: '1.5rem 1rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#1a3c8f', marginBottom: '0.25rem' }}>Latest Answer Keys</h1>
        <p style={{ color: '#64748b', fontSize: '0.9rem', marginBottom: '1.5rem' }}>Apni category select karo</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
          {categories && categories.map((category) => (
            <Link key={category.slug} href={`/answerkey/${category.slug}`} style={{ textDecoration: 'none' }}>
