    ...
    <div className="page-wrapper">
            ...
            {jobs.length > 0 ? jobs.map((job) => (
              <Link key={job.id} href={`/jobs/${job.job_categories?.slug}/${job.id}`} style={{ textDecoration: 'none' }}>
                <div style={{
                  ...
                  cursor: 'pointer', borderLeft: '4px solid #1a3c8f',
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '500px', margin: '0 auto', position: 'relative', zIndex: 10 }}>
          <h2 style={{ fontSize: '2rem', fontWeight: 900, color: 'white', textAlign: 'center', marginBottom: '8px' }}>Subscription Plan</h2>
          <p style={{ color: 'rgba(191,219,254,0.8)', textAlign: 'center', marginBottom: '2.5rem', fontSize: '0.9rem' }}>Mock Test + Previous Year Papers unlock karo</p>
          <div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: '24px', padding: '2.5rem', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <span style={{ background: '#dcfce7', color: '#166534', padding: '4px 16px', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 700, display: 'inline-block', marginTop: '8px' }}>
                ✅ 7 Din Free Trial
              </span>
            </div>
            <ul style={{ listStyle: 'none', marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {['Unlimited Mock Tests', 'All Previous Year Papers', 'Detailed Solutions', 'Performance Analysis'].map((item, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#374151' }}>
                  <span style={{ width: '22px', height: '22px', background: '#dcfce7', color: '#16a34a', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 900, flexShrink: 0 }}>✓</span>
                  {item}
                </li>
              ))}
            </ul>
            <Link href="/login" style={{ display: 'block', textAlign: 'center', textDecoration: 'none', background: 'linear-gradient(135deg, #f97316, #fb923c)', color: 'white', padding: '16px', borderRadius: '12px', fontWeight: 700, fontSize: '1rem' }}>
              🚀 Free Trial Shuru Karo
            </Link>
          </div>
        </div>
      ...
      <section style={{ padding: '5rem 1rem', background: '#f0f4ff' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: 900, color: '#1a3c8f', textAlign: 'center', marginBottom: '8px' }}>Kaise Kaam Karta Hai?</h2>
          <p style={{ color: '#64748b', textAlign: 'center', marginBottom: '3rem', fontSize: '0.9rem' }}>3 simple steps me shuru karo</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem' }}>
            {[
              ...
              <div key={index} style={{ background: 'white', borderRadius: '20px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', border: `2px solid ${item.border}`, textAlign: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block', marginBottom: '1rem' }}>
                  <div style={{ width: '72px', height: '72px', borderRadius: '18px', background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', margin: '0 auto' }}>
                    {item.icon}
                  </div>
                  <span style={{ position: 'absolute', top: '-8px', right: '-8px', width: '24px', height: '24px', background: '#1a3c8f', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 900 }}>
                    {item.step}
                  </span>
                </div>
      ...
      <section style={{ padding: '3rem 1rem', background: '#fff7ed' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <div style={{ background: 'linear-gradient(135deg, white, #f0f4ff)', borderRadius: '24px', padding: '2.5rem', boxShadow: '0 8px 30px rgba(26,60,143,0.1)', border: '2px solid #dbeafe', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem' }}>
            <div>
              <h2 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#1a3c8f', marginBottom: '8px' }}>🏆 Live Test Me Join Karo</h2>
              <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Sirf ₹9 me participate karo — Books, Laptops aur aur bhi prizes!</p>
            </div>
            <Link href="/dashboard/live-test" style={{ background: 'linear-gradient(135deg, #f97316, #fb923c)', color: 'white', padding: '16px 36px', borderRadius: '14px', fontWeight: 800, fontSize: '1.1rem', textDecoration: 'none', boxShadow: '0 8px 25px rgba(249,115,22,0.4)', flexShrink: 0 }}>
              Join Karo — ₹9 Only
            </Link>
          </div>
        </div>
