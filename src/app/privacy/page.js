...
export default function PrivacyPage() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const { data } = await supabase.from('site_settings').select('value').eq('key', 'privacy_policy').single()
  ...
  return (
    <div className="page-wrapper">
      ...
      <main style={{ flex: 1, maxWidth: '700px', margin: '0 auto', width: '100%', padding: '1.5rem 1rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#1a3c8f', marginBottom: '1.5rem' }}>🔒 Privacy Policy</h1>
        {loading ? (
